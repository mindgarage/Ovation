...
class TestQuora(object):
    @classmethod
    def setup_class(self):
        self.quora = Quora()
        self.quora.train.open()
        self.quora.validation.open()
        self.quora.test.open()

    @classmethod
    def teardown_class(self):
        self.quora.train.close()
        self.quora.validation.close()
        self.quora.test.close()
    ...
    def teardown(self):
        if 'test' in self.quora.vocab_path:
            os.remove(self.quora.vocab_path)
        if 'test' in self.quora.metadata_path:
            os.remove(self.quora.metadata_path)
        if 'test' in self.quora.w2v_path:
            os.remove(self.quora.w2v_path)

    def create_vocab(self, min_frequency, tokenizer, downcase,
                     max_vocab_size, name):
        self.quora.create_vocabulary(min_frequency=min_frequency,
        ...
        in_new_vocab = new_vocab_file in self.quora.vocab_path
        in_new_w2v = new_w2v_file in self.quora.w2v_path
        in_new_metadata = new_metadata_file in self.quora.metadata_path
    ...
    def test_init(self):
        self.quora = Quora()
        assert_not_equal(self.quora, None)
        assert_equal(self.quora.dataset_name, 'Semantic Text Similarity - All')
        assert_equal(self.quora.test_split, 'large')
        assert_equal(self.quora.vocab_size, 62451)
        assert_equal(self.quora.w2v.shape[0], 62451)
        assert_equal(self.quora.w2v.shape[1], 300)
        assert_equal(self.quora.w2v.shape[0], len(self.quora.w2i))
        assert_equal(len(self.quora.w2i), len(self.quora.i2w))
    ...
    def test_create_vocab_max_Vocab_size(self):
        ...
        self.validate_vocabulary(in_new_vocab, in_new_w2v, in_new_metadata)
        assert_equal(self.quora.vocab_size, 24)
    ...
    def test_batch_size(self):
        train_batch = self.quora.train.next_batch()
        validation_batch = self.quora.validation.next_batch()
        test_batch = self.quora.test.next_batch()
        ...
        train_batch = self.quora.train.next_batch(100)
        validation_batch = self.quora.validation.next_batch(100)
        test_batch = self.quora.test.next_batch(100)
    ...
    def test_batch_seq_begin(self):
        train_batch = self.quora.train.next_batch(seq_begin=True)
        validation_batch = self.quora.validation.next_batch(seq_begin=True)
        test_batch = self.quora.test.next_batch(seq_begin=True)
        assert_equal(train_batch.s1[0][0], self.quora.w2i['SEQ_BEGIN'])
        assert_equal(train_batch.s2[0][0], self.quora.w2i['SEQ_BEGIN'])

        assert_equal(validation_batch.s1[0][0], self.quora.w2i['SEQ_BEGIN'])
        assert_equal(validation_batch.s2[0][0], self.quora.w2i['SEQ_BEGIN'])

        assert_equal(test_batch.s1[0][0], self.quora.w2i['SEQ_BEGIN'])
        assert_equal(test_batch.s2[0][0], self.quora.w2i['SEQ_BEGIN'])


        train_batch = self.quora.train.next_batch(seq_begin=True, raw=True)
        validation_batch = self.quora.validation.next_batch(seq_begin=True,
                                                          raw=True)
        test_batch = self.quora.test.next_batch(seq_begin=True, raw=True)
    ...
    def test_batch_pad(self):
        train_batch = self.quora.train.next_batch(pad=35)
        validation_batch = self.quora.validation.next_batch(pad=35)
        test_batch = self.quora.test.next_batch(pad=35)
    ...
    def test_batch_raw(self):
        train_batch = self.quora.train.next_batch(raw=True)
        validation_batch = self.quora.validation.next_batch(raw=True)
        test_batch = self.quora.test.next_batch(raw=True)
    ...
    def test_batch_rescale(self):
        train_batch = self.quora.train.next_batch(rescale=(5, 10))
        validation_batch = self.quora.validation.next_batch(rescale=(5, 10))
        test_batch = self.quora.test.next_batch(rescale=(5, 10))
    ...
    def validate_vocabulary(self, in_new_vocab, in_new_w2v, in_new_metadata):
        assert_equal(self.quora.w2v.shape[0], len(self.quora.w2i))
        assert_equal(len(self.quora.w2i), len(self.quora.i2w))
        assert os.path.exists(self.quora.vocab_path) == True
        assert in_new_vocab == True
        assert os.path.exists(self.quora.w2v_path) == True
        assert in_new_w2v == True
        assert os.path.exists(self.quora.metadata_path) == True
