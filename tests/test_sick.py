...
class TestSick(object):
    @classmethod
    def setup_class(self):
        self.sick = Sick()
        self.sick.train.open()
        self.sick.validation.open()
        self.sick.test.open()

    @classmethod
    def teardown_class(self):
        self.sick.train.close()
        self.sick.validation.close()
        self.sick.test.close()
    ...
    def teardown(self):
        if 'test' in self.sick.vocab_path:
            os.remove(self.sick.vocab_path)
        if 'test' in self.sick.metadata_path:
            os.remove(self.sick.metadata_path)
        if 'test' in self.sick.w2v_path:
            os.remove(self.sick.w2v_path)

    def create_vocab(self, min_frequency, tokenizer, downcase,
                     max_vocab_size, name):
        self.sick.create_vocabulary(min_frequency=min_frequency,
        ...
        in_new_vocab = new_vocab_file in self.sick.vocab_path
        in_new_w2v = new_w2v_file in self.sick.w2v_path
        in_new_metadata = new_metadata_file in self.sick.metadata_path
    ...
    def test_init(self):
        self.sick = Sick()
        assert_not_equal(self.sick, None)
        assert_equal(self.sick.dataset_name, 'Semantic Text Similarity - All')
        assert_equal(self.sick.test_split, 'large')
        assert_equal(self.sick.vocab_size, 62451)
        assert_equal(self.sick.w2v.shape[0], 62451)
        assert_equal(self.sick.w2v.shape[1], 300)
        assert_equal(self.sick.w2v.shape[0], len(self.sick.w2i))
        assert_equal(len(self.sick.w2i), len(self.sick.i2w))
    ...
    def test_create_vocab_max_Vocab_size(self):
        ...
        self.validate_vocabulary(in_new_vocab, in_new_w2v, in_new_metadata)
        assert_equal(self.sick.vocab_size, 24)
    ...
    def test_batch_size(self):
        train_batch = self.sick.train.next_batch()
        validation_batch = self.sick.validation.next_batch()
        test_batch = self.sick.test.next_batch()
        ...
        train_batch = self.sick.train.next_batch(100)
        validation_batch = self.sick.validation.next_batch(100)
        test_batch = self.sick.test.next_batch(100)
    ...
    def test_batch_seq_begin(self):
        train_batch = self.sick.train.next_batch(seq_begin=True)
        validation_batch = self.sick.validation.next_batch(seq_begin=True)
        test_batch = self.sick.test.next_batch(seq_begin=True)
        assert_equal(train_batch.s1[0][0], self.sick.w2i['SEQ_BEGIN'])
        assert_equal(train_batch.s2[0][0], self.sick.w2i['SEQ_BEGIN'])

        assert_equal(validation_batch.s1[0][0], self.sick.w2i['SEQ_BEGIN'])
        assert_equal(validation_batch.s2[0][0], self.sick.w2i['SEQ_BEGIN'])

        assert_equal(test_batch.s1[0][0], self.sick.w2i['SEQ_BEGIN'])
        assert_equal(test_batch.s2[0][0], self.sick.w2i['SEQ_BEGIN'])


        train_batch = self.sick.train.next_batch(seq_begin=True, raw=True)
        validation_batch = self.sick.validation.next_batch(seq_begin=True,
                                                          raw=True)
        test_batch = self.sick.test.next_batch(seq_begin=True, raw=True)
    ...
    def test_batch_pad(self):
        train_batch = self.sick.train.next_batch(pad=35)
        validation_batch = self.sick.validation.next_batch(pad=35)
        test_batch = self.sick.test.next_batch(pad=35)
    ...
    def test_batch_raw(self):
        train_batch = self.sick.train.next_batch(raw=True)
        validation_batch = self.sick.validation.next_batch(raw=True)
        test_batch = self.sick.test.next_batch(raw=True)
    ...
    def test_batch_rescale(self):
        train_batch = self.sick.train.next_batch(rescale=(5, 10))
        validation_batch = self.sick.validation.next_batch(rescale=(5, 10))
        test_batch = self.sick.test.next_batch(rescale=(5, 10))
    ...
    def validate_vocabulary(self, in_new_vocab, in_new_w2v, in_new_metadata):
        assert_equal(self.sick.w2v.shape[0], len(self.sick.w2i))
        assert_equal(len(self.sick.w2i), len(self.sick.i2w))
        assert os.path.exists(self.sick.vocab_path) == True
        assert in_new_vocab == True
        assert os.path.exists(self.sick.w2v_path) == True
        assert in_new_w2v == True
        assert os.path.exists(self.sick.metadata_path) == True
