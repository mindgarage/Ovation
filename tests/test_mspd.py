...
class TestMSPD(object):
    @classmethod
    def setup_class(self):
        self.mspd = MSPD()
        self.mspd.train.open()
        self.mspd.validation.open()
        self.mspd.test.open()

    @classmethod
    def teardown_class(self):
        self.mspd.train.close()
        self.mspd.validation.close()
        self.mspd.test.close()
    ...
    def teardown(self):
        if 'test' in self.mspd.vocab_path:
            os.remove(self.mspd.vocab_path)
        if 'test' in self.mspd.metadata_path:
            os.remove(self.mspd.metadata_path)
        if 'test' in self.mspd.w2v_path:
            os.remove(self.mspd.w2v_path)

    def create_vocab(self, min_frequency, tokenizer, downcase,
                     max_vocab_size, name):
        self.mspd.create_vocabulary(min_frequency=min_frequency,
        ...
        in_new_vocab = new_vocab_file in self.mspd.vocab_path
        in_new_w2v = new_w2v_file in self.mspd.w2v_path
        in_new_metadata = new_metadata_file in self.mspd.metadata_path

        return new_vocab_file, new_w2v_file, new_metadata_file, in_new_vocab,\
               in_new_w2v, in_new_metadata

    def test_init(self):
        assert_not_equal(self.mspd, None)
        assert_equal(self.mspd.dataset_name, 'Semantic Text Similarity - All')
        assert_equal(self.mspd.test_split, 'large')
        assert_equal(self.mspd.vocab_size, 62451)
        assert_equal(self.mspd.w2v.shape[0], 62451)
        assert_equal(self.mspd.w2v.shape[1], 300)
        assert_equal(self.mspd.w2v.shape[0], len(self.mspd.w2i))
        assert_equal(len(self.mspd.w2i), len(self.mspd.i2w))
    ...
    def test_create_vocab_max_Vocab_size(self):
        ...
        self.validate_vocabulary(in_new_vocab, in_new_w2v, in_new_metadata)
        assert_equal(self.mspd.vocab_size, 24)
    ...
    def test_batch_size(self):
        train_batch = self.mspd.train.next_batch()
        validation_batch = self.mspd.validation.next_batch()
        test_batch = self.mspd.test.next_batch()
        ...
        train_batch = self.mspd.train.next_batch(100)
        validation_batch = self.mspd.validation.next_batch(100)
        test_batch = self.mspd.test.next_batch(100)
    ...
    def test_batch_seq_begin(self):
        train_batch = self.mspd.train.next_batch(seq_begin=True)
        validation_batch = self.mspd.validation.next_batch(seq_begin=True)
        test_batch = self.mspd.test.next_batch(seq_begin=True)
        assert_equal(train_batch.s1[0][0], self.mspd.w2i['SEQ_BEGIN'])
        assert_equal(train_batch.s2[0][0], self.mspd.w2i['SEQ_BEGIN'])

        assert_equal(validation_batch.s1[0][0], self.mspd.w2i['SEQ_BEGIN'])
        assert_equal(validation_batch.s2[0][0], self.mspd.w2i['SEQ_BEGIN'])

        assert_equal(test_batch.s1[0][0], self.mspd.w2i['SEQ_BEGIN'])
        assert_equal(test_batch.s2[0][0], self.mspd.w2i['SEQ_BEGIN'])


        train_batch = self.mspd.train.next_batch(seq_begin=True, raw=True)
        validation_batch = self.mspd.validation.next_batch(seq_begin=True,
                                                          raw=True)
        test_batch = self.mspd.test.next_batch(seq_begin=True, raw=True)
    ...
    def test_batch_pad(self):
        train_batch = self.mspd.train.next_batch(pad=35)
        validation_batch = self.mspd.validation.next_batch(pad=35)
        test_batch = self.mspd.test.next_batch(pad=35)
    ...
    def test_batch_raw(self):
        train_batch = self.mspd.train.next_batch(raw=True)
        validation_batch = self.mspd.validation.next_batch(raw=True)
        test_batch = self.mspd.test.next_batch(raw=True)
    ...
    def test_batch_rescale(self):
        train_batch = self.mspd.train.next_batch(rescale=(5, 10))
        validation_batch = self.mspd.validation.next_batch(rescale=(5, 10))
        test_batch = self.mspd.test.next_batch(rescale=(5, 10))
    ...
    def validate_vocabulary(self, in_new_vocab, in_new_w2v, in_new_metadata):
        assert_equal(self.mspd.w2v.shape[0], len(self.mspd.w2i))
        assert_equal(len(self.mspd.w2i), len(self.mspd.i2w))
        assert os.path.exists(self.mspd.vocab_path) == True
        assert in_new_vocab == True
        assert os.path.exists(self.mspd.w2v_path) == True
        assert in_new_w2v == True
        assert os.path.exists(self.mspd.metadata_path) == True
