...
# Predefined strings.
numbers = "(^a(?=\s)|first|second|third|fourth|fifth|sixth|seventh|eighth|nineth|tenth|one|two|three|four|five|six|seven|eight|nine|ten| \
...
exp1 = "(before|after|earlier|later|ago|of)"
...
def tag(text):
    ...
    for sqm in sqm_found:
        text = re.sub(sqm + '(?!</SIZE>)', '<SIZE>' + sqm + '</SIZE>', text)
...
def ground(tagged_text, base_date):
    ...
    timex_regex = re.compile(r'<DATE>.*?</DATE>', re.DOTALL)
    timex_found = timex_regex.findall(tagged_text)
    timex_found = map(lambda timex:re.sub(r'</?DATE.*?>', '', timex), \
                timex_found)
    ...
    for timex in timex_found:
        ...
        tagged_text = re.sub('<DATE>' + timex_ori + '</DATE>', '<DATE val=\"' \
            + timex_val + '\">' + timex_ori + '</DATE>', tagged_text)
